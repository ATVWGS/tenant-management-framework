function Validate-ConditionalAccessServicePrincipalFilter
{
	[CmdletBinding()]
	Param (
	...
	process
	{
		...
		foreach ($property in ($PSBoundParameters.GetEnumerator() | Where-Object {$_.Key -ne "Cmdlet"})) {
			if ($property.Key -eq "mode") {
                ...
                if (-not $ruleValid) {
                    throw "Rule $rule for servicePrincipalFilter is not valid. Cannot find referenced customSecurityAttributeDefinitions."
                }
                else {
                    $validated = $property.Value
                }
