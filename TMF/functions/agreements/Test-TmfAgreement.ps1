function Test-TmfAgreement
{
	...
	process
	{
		foreach ($definition in $script:desiredConfiguration[$resourceName]) {
			...
			$resource = (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl/identityGovernance/termsOfUse/agreements/?`$filter=displayName eq '{0}'" -f [System.Web.HttpUtility]::UrlEncode($definition.displayName))).Value
