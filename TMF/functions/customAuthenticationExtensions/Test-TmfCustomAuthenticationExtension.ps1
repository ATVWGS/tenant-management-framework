function Test-TmfCustomAuthenticationExtension {
    <#
		.SYNOPSIS
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			$result = @{
				...
				TenantId = $tenant.Id
				ResourceType = 'customAuthenticationExtension'
            ...
            try {
				$resource = (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl1/identity/customAuthenticationExtensions?`$filter=$($filter)")).value
			}
			catch {
				...
				1 {
					...
					if ($definition.present) {
						...
						foreach ($property in ($definition.Properties() | Where-Object {$_ -notin "present", "sourceConfig", "sourceFile", "oldNames"})) {
							...
							switch ($property) {
								{$_ -eq "authenticationConfiguration" -or $_ -eq "endpointConfiguration" -or $_ -eq "clientConfiguration"} {
									if (($definition.$property -and (-not ($resource.$property))) -or ($resource.$property -and (-not ($definition.$property)))) {
								...
								"claimsForTokenConfiguration" {
									if (Compare-Object $definition.$property.claimIdInApiResponse $resource.$property.claimIdInApiResponse) {
										$change.Actions = @{"Set" = $definition.$property}
									}
								}
								default {
