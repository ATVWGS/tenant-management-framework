function Test-TmfCustomSecurityAttributeDefinition
{
	<#
		.SYNOPSIS
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			$result = @{
				...
				TenantId = $tenant.Id
				ResourceType = 'CustomSecurityAttributeDefinition'
			...
			try {
                $resource = @()
				$resource += Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl/directory/customSecurityAttributeDefinitions/{0}" -f [System.Web.HttpUtility]::UrlEncode($definition.displayName))
			}
			catch {
				$resource = @()
			}
