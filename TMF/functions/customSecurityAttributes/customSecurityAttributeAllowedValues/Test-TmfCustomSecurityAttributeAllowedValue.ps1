function Test-TmfCustomSecurityAttributeAllowedValue
{
	<#
		.SYNOPSIS
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			$result = @{
				...
				TenantId = $tenant.Id
				ResourceType = 'CustomSecurityAttributeAllowedValue'
			...
			try {
                $resource = @()
				$resource += Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl/directory/customSecurityAttributeDefinitions/{0}/allowedValues/{1}" -f [System.Web.HttpUtility]::UrlEncode($definition.attributeId),[System.Web.HttpUtility]::UrlEncode($definition.id))
			}
			catch {
				$resource = @()
			}
			...
			switch ($resource.Count) {
				...
				1 {
					...
					if ($definition.present) {
						$changes = @()
						foreach ($property in ($definition.Properties() | Where-Object {$_ -in "isActive"})) {
