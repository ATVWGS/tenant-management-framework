function Test-TmfAttributeSet
{
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			try {
				$resource = @()
				$resource += Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl/directory/attributeSets/{0}" -f [System.Web.HttpUtility]::UrlEncode($definition.displayName))
			}
			catch {
				$resource = @()
			}
