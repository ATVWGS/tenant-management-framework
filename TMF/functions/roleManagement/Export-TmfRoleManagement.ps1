...
.DESCRIPTION
Invokes individual role management export functions for roleAssignments, roleDefinitions and roleManagementPolicies using optional scope and resource filtering. Returns nothing unless -OutPutPath is omitted; subordinate functions handle object returns.
.PARAMETER Scope
...
.EXAMPLE
Export-TmfRoleManagement -Scope AzureAD -OutPutPath C:\temp\tmf
.EXAMPLE
...
function Export-TmfRoleManagement {
    ...
    if ($tenant) { Write-PSFMessage -Level Verbose -FunctionName 'Export-TmfRoleManagement' -Message "Tenant: $($tenant.displayName) ($($tenant.id))" }
    ...
    process {
        foreach ($resourceType in $roleManagementResources) {
            ...
            $exportParams = @{ OutPutPath = $OutPutPath; Cmdlet = $Cmdlet }
            if ($Scope) { $exportParams.Scope = $Scope }
            if ($SpecificResources) { $exportParams.SpecificResources = $SpecificResources }
            if ($ForceBeta) { $exportParams.ForceBeta = $true }
            switch ($resourceType) {
                'roleAssignments'       { Export-TmfRoleAssignment       @exportParams }
                'roleDefinitions'       { Export-TmfRoleDefinition       @exportParams }
                'roleManagementPolicies'{ Export-TmfRoleManagementPolicy @exportParams }
            }
        }
    }
    end { Write-PSFMessage -Level Verbose -FunctionName 'Export-TmfRoleManagement' -Message 'Role management export complete.' }
}
