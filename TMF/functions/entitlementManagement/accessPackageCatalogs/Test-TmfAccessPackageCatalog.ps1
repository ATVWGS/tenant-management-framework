function Test-TmfAccessPackageCatalog
{
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			try {
				$resource = (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl1/identityGovernance/entitlementManagement/catalogs?`$filter=displayName eq '{0}'" -f [System.Web.HttpUtility]::UrlEncode($definition.displayName))).Value
			}
			catch {
