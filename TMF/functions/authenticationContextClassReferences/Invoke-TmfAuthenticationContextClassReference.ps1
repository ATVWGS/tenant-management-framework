function Invoke-TmfAuthenticationContextClassReference {
    <#
		.SYNOPSIS
	...
	begin
	{
		...
		if (!$script:desiredConfiguration[$resourceName]) {
			Stop-PSFFunction -String "TMF.NoDefinitions" -StringValues "authenticationContextClassReferences"
			return
		}
	...
	process
	{
        ...
        foreach ($result in $testResults) {
            ...
            switch ($result.ActionType) {
                "Create" {
                    $requestUrl = "$script:graphBaseUrl/identity/conditionalAccess/authenticationContextClassReferences"
					$requestMethod = "POST"
					$requestBody = @{						
                        ...
                        "isAvailable" = $result.DesiredConfiguration.isAvailable
					}
					try {						
                ...
                "Update" {
                    $requestUrl = "$script:graphBaseUrl/identity/conditionalAccess/authenticationContextClassReferences/{0}" -f $result.GraphResource.Id
                    $requestMethod = "PATCH"
                    $requestBody = @{}
					try {
