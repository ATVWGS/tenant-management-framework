...
.DESCRIPTION
Retrieves partner settings from Microsoft Graph (v1.0 by default; beta when -ForceBeta) resolving each partner's primary domain. Returns objects unless -OutPutPath is supplied, in which case JSON is written to crossTenantAccess/crossTenantAccessPartnerSettings/crossTenantAccessPartnerSettings.json.
...
.EXAMPLE
Export-TmfCrossTenantAccessPartnerSetting -OutPutPath C:\temp\tmf
.EXAMPLE
...
function Export-TmfCrossTenantAccessPartnerSetting {
    ...
    begin {
        ...
        $resolvePartnerPrimaryDomain = { param($tenantId) if (-not $tenantId) { return $null }; if ($tenantPrimaryDomainCache.ContainsKey($tenantId)) { return $tenantPrimaryDomainCache[$tenantId] } ; $primaryDomain=$null; $displayNameFallback=$null; try { $uri = "$graphBase/tenantRelationships/findTenantInformationByTenantId(tenantId='$tenantId')"; $resp = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop; $data = if ($resp.PSObject.Properties['value'] -and ($resp.value -isnot [string])) { $resp.value } else { $resp }; if ($data) { if ($data.PSObject.Properties['defaultDomainName']) { $primaryDomain = $data.defaultDomainName }; if ($data.PSObject.Properties['displayName']) { $displayNameFallback = $data.displayName } } } catch { Write-PSFMessage -Level Verbose -Message "Primary domain lookup failed for '$tenantId': $($_.Exception.Message)" -FunctionName 'Export-TmfCrossTenantAccessPartnerSetting' } ; if (-not $primaryDomain) { $primaryDomain = $displayNameFallback }; if (-not $primaryDomain) { $primaryDomain = $tenantId }; if (-not $primaryDomain -or $primaryDomain -eq $tenantId) { Write-PSFMessage -Level Warning -Message "Could not retrieve defaultDomainName for partner tenant '$tenantId'. Using fallback '$primaryDomain'." -FunctionName 'Export-TmfCrossTenantAccessPartnerSetting' } ; $tenantPrimaryDomainCache[$tenantId] = $primaryDomain; return $primaryDomain }
    }
    process {
        if ($SpecificResources) {
            $ids = @(); foreach ($entry in $SpecificResources) { $ids += $entry -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ } }; $ids = $ids | Select-Object -Unique
            foreach ($id in $ids) {
                $match = $partners | Where-Object { $_.tenantId -eq $id }
                if ($match) {
                    foreach ($m in $match) {
                        foreach ($prop in '@odata.context','id','@odata.etag','policyTenantId','tenantGroup','supportedClouds','migrationStatus','version','lastModifiedDateTime','applyDefaultsToDomainBasedOrganizations') { $m.PSObject.Properties.Remove($prop) | Out-Null }
                        $partnerDisplayName = & $resolvePartnerPrimaryDomain $m.tenantId
                        if ($partnerDisplayName -eq $m.tenantId -and $m.PSObject.Properties['displayName'] -and $m.displayName) { $partnerDisplayName = $m.displayName }
                ...
                } else { Write-PSFMessage -Level Warning -FunctionName 'Export-TmfCrossTenantAccessPartnerSetting' -String 'TMF.Export.NotFound' -StringValues $id,$resourceName,$tenant.displayName }
            }
        } else {
            foreach ($p in $partners) {
                foreach ($prop in '@odata.context','id','@odata.etag','policyTenantId','tenantGroup','supportedClouds','migrationStatus','version','lastModifiedDateTime','applyDefaultsToDomainBasedOrganizations') { $p.PSObject.Properties.Remove($prop) | Out-Null }
                $partnerDisplayName = & $resolvePartnerPrimaryDomain $p.tenantId
                if ($partnerDisplayName -eq $p.tenantId -and $p.PSObject.Properties['displayName'] -and $p.displayName) { $partnerDisplayName = $p.displayName }
        ...
        if (-not (Test-Path -LiteralPath $folderPath)) { New-Item -Path $folderPath -ItemType Directory -Force | Out-Null }
        $exports | ConvertTo-Json -Depth 15 | Out-File -FilePath (Join-Path $folderPath "$resourceName.json") -Encoding utf8 -Force
    }
}