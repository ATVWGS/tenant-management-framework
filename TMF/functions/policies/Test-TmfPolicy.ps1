function Test-TmfPolicy
{
	...
	process
	{
		...
		foreach ($resourceType in ($script:supportedResources.GetEnumerator() | Where-Object {$_.Value.testFunction -and $_.Value.parentType -eq "policies"} | Sort-Object {$_.Value.weight})) {
