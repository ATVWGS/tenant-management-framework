function Test-TmfClaimsMappingPolicy {
    <#
		.SYNOPSIS
	...
	process
	{
		...
		foreach ($definition in $definitions) {
            ...
            try {
				$resource = @()
				$resource += (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl1/policies/claimsMappingPolicies")).Value | Where-Object {$_.displayName -eq $definition.displayName}
			}
			catch {
                ...
                1 {
					...
					if ($definition.present) {
						...
						foreach ($property in ($definition.Properties() | Where-Object {$_ -notin "present", "sourceConfig", "sourceFile", "displayName"})) {
							...
							switch ($property) {
                                "appliesTo" {
                                    $existingAppliesTo = (Invoke-MgGraphRequest -Method GET -Uri ("$($script:graphBaseUrl1)/policies/claimsMappingPolicies/{0}/appliesTo" -f $resource.id)).Value.Id
                                    if ((-not $existingAppliesTo -and $definition.$property) -or (-not $definition.$property -and $existingAppliesTo)) {
                                        $change.Actions = @{"Set" = $definition.$property}
                                    }
                                    elseif ((-not $existingAppliesTo) -and (-not $definition.$property)) {#prevent compare of empty arrays
                                    }
                                    else {
