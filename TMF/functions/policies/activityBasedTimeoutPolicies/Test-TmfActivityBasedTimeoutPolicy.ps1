function Test-TmfActivityBasedTimeoutPolicy {
    <#
		.SYNOPSIS
	...
	process
	{
		...
		foreach ($definition in $definitions) {
			...
			$result = @{
				...
				TenantId = $tenant.Id
				ResourceType = 'activityBasedTimeoutPolicy'
            ...
            try {
				$resource = @()
				$resource += (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl1/policies/activityBasedTimeoutPolicies")).Value | Where-Object {$_.displayName -eq $definition.displayName}
			}
			catch {
