function Invoke-TmfConditionalAccessPolicy
{
	...
	process
	{
		...
		if (-not $Confirm) {
			Write-PSFMessage -Level Host -FunctionName "Invoke-TmfConditionalAccessPolicy" -String "TMF.TenantInformation" -StringValues $tenant.displayName, $tenant.Id
			...
			if ($SpecificResources) {
				Write-PSFMessage -Level Host -FunctionName "Invoke-TmfConditionalAccessPolicy" -String "TMF.Invoke.Confirmed" -StringValues "conditionalAccessPolicy configuration for resources: $($SpecificResources -join ",")"
				$testResults = Test-TmfConditionalAccessPolicy -SpecificResources $SpecificResources -RawOutput -Cmdlet $Cmdlet
			}
			elseif ($SourceFile) {
				Write-PSFMessage -Level Host -FunctionName "Invoke-TmfConditionalAccessPolicy" -String "TMF.Invoke.Confirmed" -StringValues "conditionalAccessPolicy configuration for SourceFile(s): $($SourceFile -join ",")"
				$testResults = Test-TmfConditionalAccessPolicy -SourceFile $SourceFile -RawOutput -Cmdlet $Cmdlet
			}
			elseif ($SourceConfig) {
				Write-PSFMessage -Level Host -FunctionName "Invoke-TmfConditionalAccessPolicy" -String "TMF.Invoke.Confirmed" -StringValues "conditionalAccessPolicy configuration for SourceConfig(s): $($SourceConfig -join ",")"
				$testResults = Test-TmfConditionalAccessPolicy -SourceConfig $SourceConfig -RawOutput -Cmdlet $Cmdlet
			}
			else {
				Write-PSFMessage -Level Host -FunctionName "Invoke-TmfConditionalAccessPolicy" -String "TMF.Invoke.Confirmed" -StringValues "all conditionalAccessPolicy configurations"
