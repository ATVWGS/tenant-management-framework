    ...
    process {
        ...
        if ($SpecificResources) {
            foreach ($specificResource in $SpecificResources) {
                if ($specificResource -match "\*") {
                    if ($script:desiredConfiguration[$resourceName] | Where-Object { $_.userPrincipalName -like $specificResource }) {
                        $definitions += $script:desiredConfiguration[$resourceName] | Where-Object { $_.userPrincipalName -like $specificResource }
                    } else {
                        Write-PSFMessage -Level Warning -String 'TMF.Error.SpecificResourceNotExists' -StringValues $specificResource -Tag 'failed'
                    ...
                    if ($script:desiredConfiguration[$resourceName] | Where-Object { $_.userPrincipalName -eq $specificResource }) {
                        $definitions += $script:desiredConfiguration[$resourceName] | Where-Object { $_.userPrincipalName -eq $specificResource }
                    } else {
                        Write-PSFMessage -Level Warning -String 'TMF.Error.SpecificResourceNotExists' -StringValues $specificResource -Tag 'failed'
                        $exception = New-Object System.Data.DataException("$($specificResource) not exists in Desired Configuration for $($resourceName)!")
                        $errorID = "SpecificResourceNotExists"
            ...
            $definitions = $definitions | Sort-Object -Property userPrincipalName -Unique
        } elseif ($SourceFile) {
        ...
        foreach ($definition in $definitions) {
            ...
            $result = @{
                ...
                TenantId             = $tenant.Id
                ResourceType         = 'User'
                ResourceName         = (Resolve-String -Text $definition.userPrincipalName)
                DesiredConfiguration = $definition
            }

            $filter = "(userPrincipalName eq '{0}')" -f [System.Web.HttpUtility]::UrlEncode($definition.userPrincipalName)
            $select = ($definition.Properties() | Where-Object { $_ -notin 'present', 'sourceConfig', 'sourceFile' })
            $select += 'id'

            try {
                $resource = (Invoke-MgGraphRequest -Method GET -Uri ("$script:graphBaseUrl/users?`$filter={0}&`$select={1}" -f $filter, ($select -join ","))).Value
            } catch {
            ...
            switch ($resource.Count) {
                        ...
                        foreach ($property in ($definition.Properties() | Where-Object { $_ -notin 'present', 'sourceConfig', 'sourceFile' })) {
                            if ($definition.$property -ne $resource.$property) {
                                $changes += [PSCustomObject]@{
                                    Property = $property
                                    Actions  = @{ 'Set' = $definition.$property }
                                }
                            }
                        }
                ...
                default {
                    Write-PSFMessage -Level Warning -String 'TMF.Test.MultipleResourcesError' -StringValues $resourceName, $definition.userPrincipalName -Tag 'failed'
